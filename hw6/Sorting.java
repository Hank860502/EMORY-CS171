...
public class Sorting {
	...
	public static void MergeSortNonRec(long[] a) {
		...
		int curr_size;
	  int start;
   	for (curr_size = 1; curr_size <= a.length-1; curr_size = 2*curr_size) { // 兩個兩個合起來所以2*curr_size = 1, 2, 4, 8
    // Pick starting point of different subarrays of current size
    	for (start = 0; start < a.length-1; start += 2*curr_size) { // 加上等於兩個兩個合起來所以位置2*curr_size = 1, 2, 4, 8
        // Find ending point of left subarray. mid+1 is starting 
        // point of right
        int mid = start + curr_size - 1;
        int end = min(start + 2*curr_size - 1, a.length-1);
        // Merge Subarrays arr[start...mid] & arr[mid+1...end]
        merge(a, a, start, mid, end);
     }
   }
	}
